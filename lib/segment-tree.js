...
function isIntervalSetValid() {
  if (!this.intervalSet.hasIntervals()) {
    this.emit('error', new Error('There is no interval available.'));
    return false;
  }
  return true;
}
...
util.inherits(SegmentTree, events.EventEmitter);
...
SegmentTree.prototype.push = function (start, end, data) {
  try {
    ...
    this.emit('pushed', { start: start, end: end, data: data });
  } catch (err) {
     this.emit('error', err);
  } finally {
    return this;
...
SegmentTree.prototype.build = function () {
  if (!isIntervalSetValid.call(this)) { return this; }
